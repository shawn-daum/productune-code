...
export const CASES: readonly Case[] = [
  ...
  },
  {
    ...
    run: () => {
      ...
      const status = checkPrdtHooksStatus(home)
      if (status.installed !== false) return fail(`installed=${status.installed} (only 1/8 hooks present)`)
      return ok
    },
  },
  {
    label: 'mirror present, settings.json already has all 8 prdt hooks → installed:true',
    run: () => {
      const home = makeHome(true)
      ...
      fs.writeFileSync(settingsPath(home), JSON.stringify({
        hooks: {
          SessionStart: [
            { matcher: 'startup|resume|clear', hooks: [h('prdt-session-start.sh'), h('prdt-audience-inject.sh'), h('prdt-plan-tier-inject.sh'), h('prdt-overrides-inject.sh')] },
