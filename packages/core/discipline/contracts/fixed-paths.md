...
- `docs/prd/PRD.md` holds the standing head + the ONE open version section; `docs/prd/history.md` holds the closed ones, in close order. WHICH rounds each file covers is a fact about the project and belongs to the two file heads, never to this contract — and a round recorded under a regime that predates this split (a whole-document snapshot rather than a `## v<N>.<m>` section) stays where that regime put it, named by those heads, never spliced into the `## v` run. Closing = cut the whole `## v<N>.<m>` block (heading through the line before the next `## `, or EOF) out of `PRD.md`, append it verbatim after the LAST `## v` section of `history.md` — ahead of any trailing non-version matter the file carries, so the version run stays one contiguous ascending block — open the next version section in `PRD.md`, and prove the move with a byte compare of the block (`shasum` before/after, each side rstripped: the blank line that separates two sections belongs to the FILE, not to either block, so a raw compare fails on a separator the move never touched) — a rewrite during the move breaks the immutable-record rule.
- History stays ONE human-read lump until some consumer is SHOWN to need it sliced: machine consumers read the open section, so a per-version file or an index is built on a verified need, never on the assumption of one. The history file is never named `PRD.md` and never sits on a path a PRD resolver would try — a resolver takes the FIRST match over `docs/prd/PRD.md` · `docs/PRD.md` · `PRD.md`, so a history file on any of them is served as the current PRD with nothing raising an error. Citation form for a closed section: `docs/prd/history.md#v<N>.<m>`; a dispatch `prd_path` always names the open section in `PRD.md`.
