...
- **Feature spec** is `docs/features/<feature>.md`, one file per feature, `<feature>` = the ticket's `feature:` value. A feature earns a file only on three tests TOGETHER — it is a mechanism (not a surface or area), it holds a contract that is still true today, and its facts landed as `done` tickets in ≥2 version dirs; a sub-feature starts as a section and is promoted by the same three tests with a `parent:`. Write the current contract, tag every fact `(vX~)` / `(vX~vY, replaced-by …)`, and never delete an invalidated one.
