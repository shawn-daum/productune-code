...
- **Feature spec** is `docs/features/<feature>.md`, one file per feature, named verbatim by the ticket's `feature:` value — but that value is a grouping key, not a pointer: most values carry no spec file, and that is the normal state. A feature earns a file only on three tests TOGETHER — it is a mechanism (not a surface or area), it holds a contract that is still true today — one an agent touching this area gets WRONG without reading it — and its facts landed as `done` tickets in ≥2 version dirs; a sub-feature starts as a section and is promoted by the same three tests with a `parent:`. Write the current contract, tag every fact `(vX~)` / `(vX~vY, replaced-by …)`, and never delete an invalidated one; where that file EXISTS, a ticket carrying the value reads it FIRST, before touching that feature.
- **Artifacts** the user reviews → `docs/artifacts/<version>/<slug>.<ext>` — the bucket names the version, never the filename. HTML preferred for interactive. On finalize run `prdt artifacts sync` to register it, then print the absolute path on its own line (+ a `file://` line for HTML — the rendered view).
