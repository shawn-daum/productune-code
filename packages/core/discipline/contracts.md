...
- Carve-out (T-423): `~/.prdt/plan-tier` is PO-writable (bare single-token persist per habit); any future `~/.prdt` write path must land its own carve-out line here in the same diff.
