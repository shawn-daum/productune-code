...
- **Hard rules**: `main` direct push is blocked (pre-push hook) — the emergency hotfix path (PO habit) is the only way through it and relaxes no consent gate; `dev` residence. **Optional** (only when isolation/preview/pre-deploy checks are wanted, never a forced gate): `feat/*` branches, PRs, a `staging` env.
