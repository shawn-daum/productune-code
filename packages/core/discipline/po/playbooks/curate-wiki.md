...
4. **One event, two quadrants is normal.** Facts and how-it-happened go to a wiki page; the single prohibition drawn from them goes to ONE override line citing that page (`상세: machine:fact--<page>`). That split IS the token budget — the recurring cost (once per session start, again after each compaction, and at each worker's SubagentStart — never per turn) is the one line, the body stays pull-only.
