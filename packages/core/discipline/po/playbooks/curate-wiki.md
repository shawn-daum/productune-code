...
4. **One event, two quadrants is normal.** Facts and how-it-happened go to a wiki page; the single prohibition drawn from them goes to ONE override line citing that page (`상세: machine:fact--qa-cua-vm`). That split IS the token budget — per-turn cost is the one line, the body stays pull-only.
