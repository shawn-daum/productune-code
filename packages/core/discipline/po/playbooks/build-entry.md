...
- Rendered for THIS version whenever the roster has an entry: a prior version's prototype or the DS showcase alone carries no roster of this version and is not the set. `hifi`'s skip/keep judgment governs screen work AFTER this approval, never the set — the set dispatch's `goal` names the version's screen set, and a return that skipped is set absent.
- The set is Define work — design tickets of the version, produced BEFORE this ritual, never among the tickets step 3 emits. The prototype passes one QA `ds-conformance` dispatch (its §Define screen set) before step 2 hands it — fail rows → designer re-render → re-check those rows only; the showcase takes no pass, its pick renders inside the prototype. Set absent → Define is not closed: no confirm, no stage write; dispatch the designer (menu match on `change_meta`) and re-enter; set unchecked → dispatch QA and re-enter.
...
- A regression this round's own commits caused — the broken behavior is on `dev` only, `main` never carried it — is ticketed as found into THIS version's dir, gate met or not: never the next-version fork, never `patch-cycle` (its in-build opening is for a regression `main` already carries, found while stage is `build`).

## Gate met — the default flips to next version
- The state change, one-way: the open PRD section's gate literal holds — a `prdt doctor` verdict (`verdict=clean`) is read off doctor, any other literal checked as written. No gate line → every ticket of the Build-entry slice is `done`/`dropped` — the slice = the tickets in `docs/tickets/v<N>.<m>/` at the stage write to `build`, ritual-cut or user-cut; both close lines (§After) name it, `T-<a>–T-<b>` or `slice: none`. `slice: none` with no gate line → the step-2 confirm asks the user for the gate literal and it goes on the PRD gate line before the stage write; a round with neither has nothing this section can read. Say it in the reply you observe it and put `gate met <date>` on the PRD's gate line (Triage's scope-decision log).
- From then, a product-shape ticket (Triage) is put to the user as going to the NEXT version. Ground that reverses it — a fact you check, never a weight: (1) dependency — an open ticket of THIS version (its file in `docs/tickets/v<N>.<m>/`; a PRD row counting a `backlog/` file as this version's is stale — fix the row or `git mv` the file before judging) cannot close without what the item delivers, on a record older than this fork: its `## Acceptance` names the deliverable in a meta commit before the item's ticket file existed (`prdt meta log`), or its worker's return (`blocked` · `unresolved[]` · `needs_info`) names it — the row quotes that line; `deps` indexes the ground, and an entry written at this fork is none; (2) double move — a named outside project or repo already carries what the item changes (name it and the file/heading), so a next-round landing changes it twice. `ground: none` for "important" · "systemic" · "small" · "half done" · "while we're at it". The user overriding without a ground → admitted, row says `ground: none`.
- Not this section — none is a product-shape change: a gate blocker (same work — patch it) · a regression this round caused (§Exception — this version's dir) · a regression `main` already carries, found while stage is `build` (`patch-cycle` in-build opening, its two trigger questions) · a live-verify bug at stage `ship` (the in-ship loop) · a scoped ticket's deeper cause (rewrite that ticket's premise, count unchanged).
...
- Record: one row in the open PRD section per item — What row + the ground, or Non-goals row `T-NNN → v<N>.<m+1> (gate met <date>, ground: none|<literal>)`. At Ship entry an open ticket the ship leaves behind takes the same fork (`readiness-dispatch` step 5); its row carries that ritual's date — `(ship-entry <date>, ground: …)` for `(gate met <date>, ground: …)`.

## After
- Ritual close: ONE `wiki/log.md` line — `(date) build-entry v<N>.<m>: sliced N tickets T-<a>–T-<b> (design/impl/qa/ops) · screens approved <n>` or `· screens: n/a — <reason>` — or, when the user chose to slice ad-hoc, the judged-skip line: `(date) build-entry v<N>.<m>: slice skipped — <reason> · tickets at entry T-<a>–T-<b>|none · screens approved <n>` or `· screens: n/a — <reason>`.
