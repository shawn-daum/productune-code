...
## Override align
Both override layers are injected on EVERY turn, so drift there costs tokens forever and silence hides it. The verdict target is the PROJECT layer — report one row per project-layer line, no sampling, no "looked fine". The machine layer is read as reference and is judged only in the project that owns it.
