...
describe.skipIf(!hasJq())('uninstall keep-list', () => {
  test('non-purge uninstall keeps register (+ overrides/, wiki/, prdt.env) and removes the mirror', () => {
    ...
    // T-586: the playbook-scoped store is a subdirectory of overrides/ and rides with it
    fs.mkdirSync(path.join(sb.prdtHome, 'overrides', 'playbooks'), { recursive: true })
    fs.writeFileSync(path.join(sb.prdtHome, 'overrides', 'playbooks', 'grill.md'), '- grill rule\n')
    ...
    expect(fs.readFileSync(path.join(sb.prdtHome, 'overrides', 'playbooks', 'grill.md'), 'utf8')).toBe('- grill rule\n')
